...
public class StaticInit {
	...
	// 1. 스태틱 변수 반드시 멤버 변수 위에 선언할것
	// (1) 스태틱 변수 선언(클래스 변수 선언)
	//     static 변수는 생성자가 초기화 하는 영역이 아니므로
	...
	// 3. 메소드 선언
	// (1) 접근자 / 수정자 ==> 멤버 변수 필드에 대해서 제공 클래스 변수 필드에 대해서 할 필요는 없음(여기선 count에 대해 필요 x)
...
//		System.out.println("memberCnt=" + getMemberCnt());
		 ...
		 * --------------------------------------------
		 * */
		StaticInit init = new StaticInit(); // non- static 이기 때문에 객체 생성 
		System.out.println("memberCnt=" + init.memberCnt);
		System.out.println("memberCnt=" + init.getMemberCnt());
	}
	/**
	 * 이 클래스 의 count 값과
	 * 입력 된 StaticInit 객체로 부터 memberCnt 값을 출력
	 * @param init
	 */
