 ...
 * 
 * -- 출력 --
 * 4. node stream (표준 출력 : System.out : PintStream)
 * 5. 1줄씩 출력 : println()
 * 
 * 
 * -- 정리 --
...
public class FileInput {
	...
	public static void main(String[] args) throws IOException {
		...
		// 2. 입력 fileter stream : BufferedReader
		BufferedReader br =  new BufferedReader(fr);
		
		// 3. 입력 filter stream의 줄단위 입력 메소드 사용
		//    readLine()
		String input = null;
		
		while((input = br.readLine()) !=null) {
			// 4,5
			System.out.println("파일에서 읽을 수 없는 데이터");
		}
		
		// 6. 6. 입력에서 filter stream 닫기
				br.close();
				}
}
