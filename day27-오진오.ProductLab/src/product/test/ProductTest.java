...
//			int add = 0;
//			add = dao.insertProduct(product); 
//
//			System.out.printf("%d건이 추가되었습니다.", add);
//		}
...
//		//        P001 을 가지도록 생성하여 그 인스턴스를 넘기도록 함
		...
		price.setMin(20000);
		price.setMax(50000); 
		List<Product> priceProducts = dao.selectProducts(price);   // TODO : dao 객체를 사용하여 가격대 검색 완성;
		...
		int maxpri = 40000;
		List<Product> expensivesThan = dao.selectProductsByMaxPrice(maxpri);// TODO : dao 객체를 사용하여 가격 이상 검색 완성;
		...
		int minpri = 50000;
		List<Product> cheaps = dao.selectProductsByMinPrice(minpri);// TODO : dao 객체를 사용하여 가격 이하 검색 완성;
		for (Product product: cheaps) {
			System.out.println(product);			
		}
		
		// TODO : 가장 비싼 제품목록 검색
		List<Product> mostExpensives = dao.selectMostExpensive();// TODO : dao 객체를 사용하여 가장 비싼 검색 완성;
		for (Product product: mostExpensives) {
			System.out.println(product);			
		}
		
		// TODO : 가장 재고 많은 제품목록 검색
		List<Product> mostQuantities = dao.selectMostQuantity();// TODO : dao 객체를 사용하여 가장 재고 많은 검색 완성;
		...
		//        P005 을 가지도록 생성
		PrimaryKey del = new PrimaryKey();
		del.setCode("P005");
		// TODO : dao 에 deleteProduct(PriamryKey pk) 메소드를 호출하며
		//        위에서 생성한 PrimaryKey 인스턴스 전달
		dao.deleteProduct(del);
		
		
		
		// 삭제 -----------------------------------------------------------------------
		// TODO : P005 가 삭제된 전체 목록 조회
		List<Product> allProducts = dao.selectProducts();// TODO : dao 객체를 사용하여 전체 목록 검색 완성;
